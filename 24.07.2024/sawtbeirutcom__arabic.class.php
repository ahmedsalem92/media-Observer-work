...
class sawtbeirutcom__arabic extends plugin_base
{
	...
	{
		// if the the array of index zero is true and isset $link
		if (isset($this->logic['list1'][0]) && $this->logic['list1'][0]) {
			if (isset($link)) {
				$element = $this->logic['list1'][0];
				$type = $element['type'];
				$regexp = $element['regexp'];
				$append_domain = $element['append_domain'];
				$process_link = $element['process_link'];
				$processedLink = $this->$process_link($link);

				// Make while loop to repeat the array of index 1 in the array of list until to reach the last one
				$index = 1;
				while ($index < count($this->logic['list1'])) {
					$element = $this->logic['list1'][$index];
					$type = $element['type'];
					$regexp = $element['regexp'];
					$append_domain = $element['append_domain'];
					$process_link = $element['process_link'];
