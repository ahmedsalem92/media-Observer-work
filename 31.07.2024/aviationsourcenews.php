...
class thegulfheraldcom extends plugin_base
{
	...
	protected $logic = array(
		...
		'article' => array(
			'headline' => '/<h1[^<]*>(.*)<\/h1>/Uis',
			'content' => '/<div class="entry-content rbct clearfix.*">(.*)(?:<div class="smeta-bottom meta-text">|<div class="efoot efoot-commas h5">)/Uis',
			'author' => false,
	...
	{
		$content = preg_replace('/>(https:\/\/creativecommons\.org\/.*)</Uis', '', $content);
		$content = preg_replace('/(<p>Click the banner to subscribe to our weekly Emergencies and Incidents newsletter.<\/p>)</Uis', '', $content);
		$content = preg_replace('/(<p>Click the photo to join our WhatsApp channel so then you can stay up to date with everything going on in the aviation industry!<\/p>)</Uis', '', $content);
		return $content;
	}
